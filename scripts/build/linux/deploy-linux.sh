...
# Copy executable (deploy as GeneralsX for run script compatibility)
...
patchelf --set-rpath '$ORIGIN' "${RUNTIME_DIR}/GeneralsX" 2>/dev/null || {
...
echo "  or: cd ~/GeneralsX/Generals && ./run.sh -win"
