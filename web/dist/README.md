...
`GeneralsXZH.js`, `GeneralsXZH.wasm` and `GeneralsXZH.data` are build output, and they are
...
Building them in CI is not a realistic alternative: it needs Emscripten, vcpkg, ninja and a full
engine compile.
...
cp build/webgpu/GeneralsMD/GeneralsXZH.{js,wasm,data} web/public/
```
