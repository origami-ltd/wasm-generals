(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&o(a)}).observe(document,{childList:!0,subtree:!0});function n(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(s){if(s.ep)return;s.ep=!0;const r=n(s);fetch(s.href,r)}})();const Z="modulepreload",N=function(e){return"/"+e},D={},Y=function(t,n,o){let s=Promise.resolve();if(n&&n.length>0){let a=function(d){return Promise.all(d.map(h=>Promise.resolve(h).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const i=document.querySelector("meta[property=csp-nonce]"),l=i?.nonce||i?.getAttribute("nonce");s=a(n.map(d=>{if(d=N(d),d in D)return;D[d]=!0;const h=d.endsWith(".css"),m=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${m}`))return;const f=document.createElement("link");if(f.rel=h?"stylesheet":Z,h||(f.as="script"),f.crossOrigin="",f.href=d,l&&f.setAttribute("nonce",l),document.head.appendChild(f),h)return new Promise((C,x)=>{f.addEventListener("load",C),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(a){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=a,window.dispatchEvent(i),!i.defaultPrevented)throw a}return s.then(a=>{for(const i of a||[])i.status==="rejected"&&r(i.reason);return t().catch(r)})},g=4*1024*1024,K=192*1024*1024,V=`
  postMessage("ready");
  ...
  };`;class J{constructor(t){if(this.onError=t,!crossOriginIsolated){this.ready=Promise.resolve();return}this.worker=new Worker(URL.createObjectURL(new Blob([V],{type:"text/javascript"}))),this.buffer=new SharedArrayBuffer(g+8),this.ready=new Promise(n=>this.worker?.addEventListener("message",n,{once:!0}))}cache=new Map;worker=null;buffer=null;ready;fetchChunkSync(t,n,o){if(!this.worker||!this.buffer)return this.onError("SharedArrayBuffer unavailable: archives cannot stream."),new Uint8Array(0);const s=new Int32Array(this.buffer,0,2);Atomics.store(s,0,0);const r=n*g;this.worker.postMessage({url:o?"":new URL(t,location.href).href,handle:o,start:r,end:r+g-1,sab:this.buffer});const a=Date.now()+6e4;for(;Atomics.load(s,0)===0;)if(Date.now()>a)return this.onError(`Archive fetch timed out: ${t} chunk ${n}`),new Uint8Array(0);return Atomics.load(s,0)!==1?(this.onError(`Archive fetch failed: ${t} chunk ${n}`),new Uint8Array(0)):new Uint8Array(this.buffer.slice(8,8+(s[1]??0)))}takeChunk(t,n,o){const s=`${t}#${n}`,r=this.cache.get(s);if(r)return this.cache.delete(s),this.cache.set(s,r),r;const a=this.fetchChunkSync(t,n,o);this.cache.set(s,a);let i=0;for(const l of this.cache.values())i+=l.length;for(;i>K&&this.cache.size>1;){const l=this.cache.keys().next().value;i-=this.cache.get(l)?.length??0,this.cache.delete(l)}return a}mount(t,n){const o=t.FS;o.mkdirTree(n.mount);const s=o.createFile(n.mount,n.name,{},!0,!1),r=n.size;Object.defineProperty(s,"usedBytes",{get:()=>r}),s.stream_ops={llseek:(a,i,l)=>{let d=i;if(l===1?d+=a.position:l===2&&(d=r+i),d<0)throw new o.ErrnoError(28);return d},read:(a,i,l,d,h)=>{const m=Math.min(r,h+d);if(h>=m)return 0;const f=Math.floor(h/g),C=Math.floor((m-1)/g);let x=0;for(let w=f;w<=C;w+=1){const X=this.takeChunk(n.url,w,n.handle),S=w*g,M=Math.max(h,S)-S,P=Math.min(m,S+X.length)-S;if(P<=M)break;i.set(X.subarray(M,P),l+x),x+=P-M}return x}}}}async function Q(){return await(await fetch("/GeneralsXAssets")).json()}async function ee(e,t=3){const n=new Map,o=async(s,r)=>{let a=!1,i=!1;const l=[];for await(const[d,h]of s.entries())h.kind==="directory"?l.push(h):d.toLowerCase().endsWith(".big")&&(/zh\.big$/i.test(d)?a=!0:i=!0);if(a&&!n.has("GeneralsZH")?n.set("GeneralsZH",s):i&&!a&&!n.has("Generals")&&n.set("Generals",s),!(n.size===2||r>=t)){for(const d of l)if(await o(d,r+1),n.size===2)return}};return await o(e,0),n}async function te(){const t=(await new Promise((s,r)=>{const a=indexedDB.open("generalsx",1);a.onupgradeneeded=()=>a.result.createObjectStore("handles"),a.onsuccess=()=>s(a.result),a.onerror=()=>r(a.error)})).transaction("handles","readonly").objectStore("handles"),n=s=>new Promise(r=>{const a=t.get(s);a.onsuccess=()=>r(a.result),a.onerror=()=>r(void 0)}),o=[];for(const s of["GeneralsZH","Generals"]){const r=await n(s);if(r){if(await r.queryPermission?.({mode:"read"})!=="granted"&&await r.requestPermission?.({mode:"read"})!=="granted")return[];for await(const[a,i]of r.entries()){if(!a.toLowerCase().endsWith(".big")||i.kind!=="file")continue;const l=await i.getFile();o.push({mount:`/${s}`,name:a,url:`local:${s}/${a}`,size:l.size,handle:i})}}}return o}const se=`
  ...
  <p>On macOS or Linux, pick the folder holding the game's <code class="text-hud-warm">.big</code> archives
    ...
    </main>`}const c=e=>document.getElementById(e);ne(c("app"));const u=c("canvas"),E=c("frame"),I=c("stage"),G=c("output"),F=c("status"),b=c("cursor-overlay"),_=new URLSearchParams(location.search),R="/home/web_user/.local/share/GeneralsX/GeneralsZH",re=`Resolution = 1280 720
