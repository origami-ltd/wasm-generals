(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function t(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(s){if(s.ep)return;s.ep=!0;const a=t(s);fetch(s.href,a)}})();const z="modulepreload",Y=function(e){return"/"+e},H={},K=function(n,t,r){let s=Promise.resolve();if(t&&t.length>0){let i=function(d){return Promise.all(d.map(h=>Promise.resolve(h).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),l=o?.nonce||o?.getAttribute("nonce");s=i(t.map(d=>{if(d=Y(d),d in H)return;H[d]=!0;const h=d.endsWith(".css"),m=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${m}`))return;const f=document.createElement("link");if(f.rel=h?"stylesheet":z,h||(f.as="script"),f.crossOrigin="",f.href=d,l&&f.setAttribute("nonce",l),document.head.appendChild(f),h)return new Promise((P,w)=>{f.addEventListener("load",P),f.addEventListener("error",()=>w(new Error(`Unable to preload CSS for ${d}`)))})}))}function a(i){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=i,window.dispatchEvent(o),!o.defaultPrevented)throw i}return s.then(i=>{for(const o of i||[])o.status==="rejected"&&a(o.reason);return n().catch(a)})},g=4*1024*1024,V=192*1024*1024,J=`
  postMessage("ready");
  ...
  };`;class Q{constructor(n){if(this.onError=n,!crossOriginIsolated){this.ready=Promise.resolve();return}this.worker=new Worker(URL.createObjectURL(new Blob([J],{type:"text/javascript"}))),this.buffer=new SharedArrayBuffer(g+8),this.ready=new Promise(t=>this.worker?.addEventListener("message",t,{once:!0}))}cache=new Map;worker=null;buffer=null;ready;fetchChunkSync(n,t,r){if(!this.worker||!this.buffer)return this.onError("SharedArrayBuffer unavailable: archives cannot stream."),new Uint8Array(0);const s=new Int32Array(this.buffer,0,2);Atomics.store(s,0,0);const a=t*g;this.worker.postMessage({url:r?"":new URL(n,location.href).href,handle:r,start:a,end:a+g-1,sab:this.buffer});const i=Date.now()+6e4;for(;Atomics.load(s,0)===0;)if(Date.now()>i)return this.onError(`Archive fetch timed out: ${n} chunk ${t}`),new Uint8Array(0);return Atomics.load(s,0)!==1?(this.onError(`Archive fetch failed: ${n} chunk ${t}`),new Uint8Array(0)):new Uint8Array(this.buffer.slice(8,8+(s[1]??0)))}takeChunk(n,t,r){const s=`${n}#${t}`,a=this.cache.get(s);if(a)return this.cache.delete(s),this.cache.set(s,a),a;const i=this.fetchChunkSync(n,t,r);this.cache.set(s,i);let o=0;for(const l of this.cache.values())o+=l.length;for(;o>V&&this.cache.size>1;){const l=this.cache.keys().next().value;o-=this.cache.get(l)?.length??0,this.cache.delete(l)}return i}mount(n,t){const r=n.FS;r.mkdirTree(t.mount);const s=r.createFile(t.mount,t.name,{},!0,!1),a=t.size;Object.defineProperty(s,"usedBytes",{get:()=>a}),s.stream_ops={llseek:(i,o,l)=>{let d=o;if(l===1?d+=i.position:l===2&&(d=a+o),d<0)throw new r.ErrnoError(28);return d},read:(i,o,l,d,h)=>{const m=Math.min(a,h+d);if(h>=m)return 0;const f=Math.floor(h/g),P=Math.floor((m-1)/g);let w=0;for(let k=f;k<=P;k+=1){const F=this.takeChunk(t.url,k,t.handle),L=k*g,I=Math.max(h,L)-L,G=Math.min(m,L+F.length)-L;if(G<=I)break;o.set(F.subarray(I,G),l+w),w+=G-I}return w}}}}async function ee(){return await(await fetch("/GeneralsXAssets")).json()}async function te(e,n=3){const t=new Map,r=async(s,a)=>{let i=!1,o=!1;const l=[];for await(const[d,h]of s.entries())h.kind==="directory"?l.push(h):d.toLowerCase().endsWith(".big")&&(/zh\.big$/i.test(d)?i=!0:o=!0);if(i&&!t.has("GeneralsZH")?t.set("GeneralsZH",s):o&&!i&&!t.has("Generals")&&t.set("Generals",s),!(t.size===2||a>=n)){for(const d of l)if(await r(d,a+1),t.size===2)return}};return await r(e,0),t}async function ne(){try{return await se()}catch(e){return console.debug("local archives unavailable",e),[]}}async function se(){const n=(await new Promise((a,i)=>{const o=indexedDB.open("generalsx",1);o.onupgradeneeded=()=>o.result.createObjectStore("handles"),o.onsuccess=()=>a(o.result),o.onerror=()=>i(o.error)})).transaction("handles","readonly").objectStore("handles"),t=a=>new Promise(i=>{const o=n.get(a);o.onsuccess=()=>i(o.result),o.onerror=()=>i(void 0)}),r={GeneralsZH:t("GeneralsZH"),Generals:t("Generals")},s=[];for(const a of["GeneralsZH","Generals"]){const i=await r[a];if(i){if(await i.queryPermission?.({mode:"read"})!=="granted"&&await i.requestPermission?.({mode:"read"})!=="granted")return[];for await(const[o,l]of i.entries()){if(!o.toLowerCase().endsWith(".big")||l.kind!=="file")continue;const d=await l.getFile();s.push({mount:`/${a}`,name:o,url:`local:${a}/${o}`,size:d.size,handle:l})}}}return s}async function re(){try{const e=await new Promise((n,t)=>{const r=indexedDB.open("generalsx",1);r.onupgradeneeded=()=>r.result.createObjectStore("handles"),r.onsuccess=()=>n(r.result),r.onerror=()=>t(r.error)});return await new Promise(n=>{const t=e.transaction("handles","readonly").objectStore("handles").count();t.onsuccess=()=>n(t.result>0),t.onerror=()=>n(!1)})}catch{return!1}}const ae=`
  ...
  <p>On macOS or Linux, pick the folder holding the game's <code class="text-hud-warm">.big</code> archives
    ...
    </main>`}const c=e=>document.getElementById(e);oe(c("app"));const u=c("canvas"),E=c("frame"),R=c("stage"),O=c("output"),U=c("status"),x=c("cursor-overlay"),v=new URLSearchParams(location.search),X="/home/web_user/.local/share/GeneralsX/GeneralsZH",ie=`Resolution = 1280 720
