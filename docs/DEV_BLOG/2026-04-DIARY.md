...
Root cause:
- during upstream conflict resolution, the integer `from_chars` block was accidentally dropped while keeping the new error checks.
- this left `ec` and `result` out of scope for non-floating-point template instantiations.

Impact:
- failed Linux jobs (`Generals`, `GeneralsMD`, and Flatpak variants) all at the same file/line region in `INI.cpp`.

Validation:
- manual build confirmed as successful after the fix.
...
- Imported 13 upstream commits including INI parsing, camera/view, energy, pathfinder, projectile, and input fixes.
- Accepted WW3D2 DX8 wrapper file move to unified Core layout (unify commit), keeping GeneralsX DXVK-oriented include behavior.
- Merged INI parser changes to keep both plus-prefix parsing and Apple-compatible float parsing path.
- Preserved GeneralsX platform-sensitive PCH behavior in WW3D2 CMakeLists (Windows-only PCH guard).

Build blockers fixed after sync:
- Updated SDL3GameEngine factory signatures in both game variants to match new pure-virtual `GameEngine` contract (`createRadar(Bool)`, `createAudioManager(Bool)`).
- Fixed Generals base `sortingrenderer.cpp` to use unified `RenderStateStruct` field names (`vertex_buffers`, `vertex_buffer_types`) and release all vertex streams correctly.

Validation outcome:
- macOS configure completed successfully (`macos-vulkan`).
- Generals base build completed successfully after renderer fix.
- Zero Hour abstract-class construction error in SDL3 main path was resolved by signature alignment.
