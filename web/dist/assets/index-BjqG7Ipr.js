(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const z="modulepreload",N=function(e){return"/"+e},T={},Y=function(s,t,i){let n=Promise.resolve();if(t&&t.length>0){let o=function(d){return Promise.all(d.map(h=>Promise.resolve(h).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),l=a?.nonce||a?.getAttribute("nonce");n=o(t.map(d=>{if(d=N(d),d in T)return;T[d]=!0;const h=d.endsWith(".css"),m=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${m}`))return;const f=document.createElement("link");if(f.rel=h?"stylesheet":z,h||(f.as="script"),f.crossOrigin="",f.href=d,l&&f.setAttribute("nonce",l),document.head.appendChild(f),h)return new Promise((M,b)=>{f.addEventListener("load",M),f.addEventListener("error",()=>b(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(o){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=o,window.dispatchEvent(a),!a.defaultPrevented)throw o}return n.then(o=>{for(const a of o||[])a.status==="rejected"&&r(a.reason);return s().catch(r)})},g=4*1024*1024,K=192*1024*1024,V=`
  postMessage("ready");
  ...
  };`;class J{constructor(s){if(this.onError=s,!crossOriginIsolated){this.ready=Promise.resolve();return}this.worker=new Worker(URL.createObjectURL(new Blob([V],{type:"text/javascript"}))),this.buffer=new SharedArrayBuffer(g+8),this.ready=new Promise(t=>this.worker?.addEventListener("message",t,{once:!0}))}cache=new Map;worker=null;buffer=null;ready;fetchChunkSync(s,t,i){if(!this.worker||!this.buffer)return this.onError("SharedArrayBuffer unavailable: archives cannot stream."),new Uint8Array(0);const n=new Int32Array(this.buffer,0,2);Atomics.store(n,0,0);const r=t*g;this.worker.postMessage({url:i?"":new URL(s,location.href).href,handle:i,start:r,end:r+g-1,sab:this.buffer});const o=Date.now()+6e4;for(;Atomics.load(n,0)===0;)if(Date.now()>o)return this.onError(`Archive fetch timed out: ${s} chunk ${t}`),new Uint8Array(0);return Atomics.load(n,0)!==1?(this.onError(`Archive fetch failed: ${s} chunk ${t}`),new Uint8Array(0)):new Uint8Array(this.buffer.slice(8,8+(n[1]??0)))}takeChunk(s,t,i){const n=`${s}#${t}`,r=this.cache.get(n);if(r)return this.cache.delete(n),this.cache.set(n,r),r;const o=this.fetchChunkSync(s,t,i);this.cache.set(n,o);let a=0;for(const l of this.cache.values())a+=l.length;for(;a>K&&this.cache.size>1;){const l=this.cache.keys().next().value;a-=this.cache.get(l)?.length??0,this.cache.delete(l)}return o}mount(s,t){const i=s.FS;i.mkdirTree(t.mount);const n=i.createFile(t.mount,t.name,{},!0,!1),r=t.size;Object.defineProperty(n,"usedBytes",{get:()=>r}),n.stream_ops={llseek:(o,a,l)=>{let d=a;if(l===1?d+=o.position:l===2&&(d=r+a),d<0)throw new i.ErrnoError(28);return d},read:(o,a,l,d,h)=>{const m=Math.min(r,h+d);if(h>=m)return 0;const f=Math.floor(h/g),M=Math.floor((m-1)/g);let b=0;for(let v=f;v<=M;v+=1){const F=this.takeChunk(t.url,v,t.handle),S=v*g,P=Math.max(h,S)-S,G=Math.min(m,S+F.length)-S;if(G<=P)break;a.set(F.subarray(P,G),l+b),b+=G-P}return b}}}}async function Q(){return await(await fetch("/GeneralsXAssets")).json()}async function ee(e,s=3){const t=new Map,i=async(n,r)=>{let o=!1,a=!1;const l=[];for await(const[d,h]of n.entries())h.kind==="directory"?l.push(h):d.toLowerCase().endsWith(".big")&&(/zh\.big$/i.test(d)?o=!0:a=!0);if(o&&!t.has("GeneralsZH")?t.set("GeneralsZH",n):a&&!o&&!t.has("Generals")&&t.set("Generals",n),!(t.size===2||r>=s)){for(const d of l)if(await i(d,r+1),t.size===2)return}};return await i(e,0),t}async function te(){try{return await se()}catch(e){return console.debug("local archives unavailable",e),[]}}async function se(){const s=(await new Promise((r,o)=>{const a=indexedDB.open("generalsx",1);a.onupgradeneeded=()=>a.result.createObjectStore("handles"),a.onsuccess=()=>r(a.result),a.onerror=()=>o(a.error)})).transaction("handles","readonly").objectStore("handles"),t=r=>new Promise(o=>{const a=s.get(r);a.onsuccess=()=>o(a.result),a.onerror=()=>o(void 0)}),i={GeneralsZH:t("GeneralsZH"),Generals:t("Generals")},n=[];for(const r of["GeneralsZH","Generals"]){const o=await i[r];if(o){if(await o.queryPermission?.({mode:"read"})!=="granted"&&await o.requestPermission?.({mode:"read"})!=="granted")return[];for await(const[a,l]of o.entries()){if(!a.toLowerCase().endsWith(".big")||l.kind!=="file")continue;const d=await l.getFile();n.push({mount:`/${r}`,name:a,url:`local:${r}/${a}`,size:d.size,handle:l})}}}return n}async function ne(){try{const e=await new Promise((s,t)=>{const i=indexedDB.open("generalsx",1);i.onupgradeneeded=()=>i.result.createObjectStore("handles"),i.onsuccess=()=>s(i.result),i.onerror=()=>t(i.error)});return await new Promise(s=>{const t=e.transaction("handles","readonly").objectStore("handles").count();t.onsuccess=()=>s(t.result>0),t.onerror=()=>s(!1)})}catch{return!1}}const re=`
  ...
  <p>On macOS or Linux, pick the folder holding the game's <code class="text-hud-warm">.big</code> archives
    ...
    </main>`}const c=e=>document.getElementById(e);ae(c("app"));const u=c("canvas"),C=c("frame"),R=c("stage"),I=c("output"),X=c("status"),w=c("cursor-overlay"),E=new URLSearchParams(location.search),$="/home/web_user/.local/share/GeneralsX/GeneralsZH",oe=`Resolution = 1280 720
`,L=[],k=[];let j=Promise.resolve();function p(e){L.push(e),k.push(e),k.length>512&&k.shift(),I.value=`${k.join(`
`)}
`,I.scrollTop=I.scrollHeight}function U(e=!1){if(!L.length)return;const s=`${L.join(`
`)}
`;if(L.length=0,e){navigator.sendBeacon("/GeneralsXLog",s);return}j=j.then(()=>fetch("/GeneralsXLog",{method:"POST",body:s}).catch(()=>{}))}setInterval(()=>U(),2e3);addEventListener("pagehide",()=>U(!0));const _=e=>{X.textContent=e||X.textContent||""},q=(E.get("boot")??localStorage.getItem("generalsX.bootMode"))==="full"?"full":"fast",ie=E.get("sound")!=="0";let A=localStorage.getItem("generalsX.soundMuted")==="1";const B=q==="fast"?["-quickstart","-noshellmap"]:[];ie||B.push("-noaudio");c("boot").value=q;c("boot").addEventListener("change",e=>{localStorage.setItem("generalsX.bootMode",e.target.value),location.reload()});const O=c("aspect");O.value=localStorage.getItem("generalsX.aspect")==="4:3"?"4:3":"16:9";O.addEventListener("change",()=>{localStorage.setItem("generalsX.aspect",O.value),localStorage.setItem("generalsX.aspectApply","1"),location.reload()});c("reset").addEventListener("click",()=>{localStorage.clear(),indexedDB.deleteDatabase("generalsx"),location.reload()});c("share").addEventListener("click",async()=>{const e=c("share"),{url:s}=await(await fetch("/GeneralsXShare")).json();await navigator.clipboard?.writeText(s).catch(()=>{});const t=e.textContent;e.textContent="Link copied",setTimeout(()=>{e.textContent=t},1800)});c("firstrun-info").addEventListener("click",()=>{const e=c("firstrun-info-panel");e.hidden=!e.hidden});c("firstrun-folder").addEventListener("click",async()=>{const e=c("firstrun-folder-note"),s=window.showDirectoryPicker;if(!s){e.textContent="This browser cannot pick folders — use Chrome or Edge.";return}try{const t=await s({id:"generalsx-install",mode:"read"});e.textContent="Scanning…";const i=await ee(t);if(!i.has("GeneralsZH")){e.textContent="No Zero Hour archives (*ZH.big) under that folder — pick the install folder.";return}const r=(await new Promise((o,a)=>{const l=indexedDB.open("generalsx",1);l.onupgradeneeded=()=>l.result.createObjectStore("handles"),l.onsuccess=()=>o(l.result),l.onerror=()=>a(l.error)})).transaction("handles","readwrite").objectStore("handles");for(const[o,a]of i)r.put(a,o);e.textContent=`Found ${[...i.keys()].join(" + ")}. Starting…`,setTimeout(()=>location.replace(location.pathname),700)}catch(t){console.debug("folder selection cancelled",t),e.textContent=""}});function y(){const e=document.fullscreenElement===C,s=Math.min(e?innerWidth:R.clientWidth,innerWidth)-16,t=Math.min(e?innerHeight:R.clientHeight,innerHeight)-16,i=Math.min(s/(u.width||1),t/(u.height||1));u.style.width=`${Math.max(1,Math.floor((u.width||1)*i))}px`,u.style.height=`${Math.max(1,Math.floor((u.height||1)*i))}px`}new ResizeObserver(y).observe(R);new MutationObserver(y).observe(u,{attributes:!0,attributeFilter:["width","height"]});addEventListener("resize",y);document.addEventListener("fullscreenchange",y);c("fullscreen").addEventListener("click",()=>void C.requestFullscreen().catch(()=>{}));let D="";function W(){if(document.pointerLockElement!==u)return;const e=x?._GeneralsXMouseX?.()??-1,s=x?._GeneralsXMouseY?.()??-1,t=/url\(\s*"?([^")]+)"?\s*\)(?:\s+(\d+)\s+(\d+))?/.exec(u.style.cursor);if(t&&e>=0&&s>=0){const[,i,n="0",r="0"]=t;i!==D&&(D=i,w.src=i);const o=u.getBoundingClientRect(),a=o.width/(u.width||1),l=o.height/(u.height||1);w.hidden=!1,w.style.transform=`translate(${o.left+e*a-Number(n)}px, ${o.top+s*l-Number(r)}px)`}else w.hidden=!0;requestAnimationFrame(W)}document.addEventListener("pointerlockchange",()=>{const e=document.pointerLockElement===u;u.classList.toggle("pointer-locked",e),e?W():w.hidden=!0});u.addEventListener("contextmenu",e=>e.preventDefault());u.addEventListener("pointerdown",()=>{u.focus(),!document.pointerLockElement&&C.dataset.ready==="true"&&u.requestPointerLock()});function le(e){A=e,localStorage.setItem("generalsX.soundMuted",e?"1":"0"),x?._GeneralsXSetAudioMuted?.(e?1:0),c("sound").textContent=e?"Sound off":"Sound on"}c("sound").addEventListener("click",()=>le(!A));c("sound").textContent=A?"Sound off":"Sound on";crossOriginIsolated||_("Open this page over https:// — the browser blocks shared memory otherwise.");const H=new J(p);let x;c("cap-wasm").textContent=typeof WebAssembly=="object"?"WASM ready":"WASM missing";c("cap-webgpu").textContent="gpu"in navigator?"WebGPU ready":"WebGPU missing";const Z={canvas:u,arguments:B,print:e=>p(e),printErr:e=>p(e),setStatus:_,preRun:[e=>{if(e.addRunDependency("gx-assets"),E.get("assets")==="1"){c("firstrun").hidden=!1;return}Promise.all([te(),H.ready]).then(async([s])=>{if(s.length){for(const n of s)H.mount(e,n);p(`Streaming ${s.length} archives from your selected folders.`),e.removeRunDependency("gx-assets");return}if(await ne()){c("firstrun").hidden=!1,c("firstrun-folder-note").textContent="Click to re-allow access to your game folder.";return}const t=await Q();if(t.missing){c("firstrun").hidden=!1,p("Game archives not found — waiting for the player to point at their install.");return}for(const n of t.entries)H.mount(e,n);const i=t.entries.reduce((n,r)=>n+r.size,0);p(`Streaming ${t.entries.length} game archives (${(i/2**30).toFixed(1)} GB) on demand.`),e.removeRunDependency("gx-assets")}).catch(s=>{p(`Asset manifest failed: ${s.message}`),e.removeRunDependency("gx-assets")})},e=>{e.addRunDependency("gx-userdata");const s=e.FS;s.mkdirTree($),s.mount(e.IDBFS,{},$),s.syncfs(!0,()=>{const t=`${$}/Options.ini`;let i=!0;try{s.stat(t)}catch{i=!1}if((!i||E.get("resetOptions")==="1")&&s.writeFile(t,oe),localStorage.getItem("generalsX.aspectApply")==="1"){const n=localStorage.getItem("generalsX.aspect")==="4:3"?"Resolution = 1024 768":"Resolution = 1280 720",r=s.readFile(t,{encoding:"utf8"});s.writeFile(t,/^Resolution = /m.test(r)?r.replace(/^Resolution = .*$/m,n):`${r}${n}
`),localStorage.removeItem("generalsX.aspectApply")}setInterval(()=>s.syncfs(!1,()=>{}),1e4),addEventListener("pagehide",()=>s.syncfs(!1,()=>{})),e.removeRunDependency("gx-userdata")})}]};Z.onRuntimeInitialized=function(){if(x=this,globalThis.__gx=this,C.dataset.ready="true",c("share").hidden=!1,_("Running"),y(),A){const e=setInterval(()=>{x?._GeneralsXSetAudioMuted?.(1)&&clearInterval(e)},500)}};_("Loading…");const ce=(await Y(async()=>{const{default:e}=await import("/GeneralsXZH.js");return{default:e}},[])).default;ce(Z);
