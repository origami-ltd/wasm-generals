...
- Installs build dependencies via `apt-get` (cmake, ninja-build, meson, X11/Wayland dev libs)
- Configures CMake with `linux64-deploy` or `linux64-testing` preset
- Compiles GeneralsXZH or Generals
- Verifies native ELF binary artifacts
...
- Future: Docker optimization could replace direct ubuntu:latest build (currently simpler for GitHub Actions)

**Future Phases** (in `docs/WORKDIR/planning/github-actions-strategy.md`):
1. ✅ macOS build-on-demand, standalone (DONE)
2. ✅ Linux build-on-demand, standalone (DONE)
3. 🔄 Enable PR validation (uncomment `on.push`/`on.pull_request` in ci.yml)
4. 🔄 Add replay determinism validation job (cross-platform hash comparison)

**References**:
- `.github/workflows/build-linux.yml` - Linux build workflow (standalone + callable)
- `.github/workflows/ci.yml` - Main CI orchestrator (now calls both macOS and Linux)
- `docs/WORKDIR/planning/github-actions-strategy.md` - Master strategy document
